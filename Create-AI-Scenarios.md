...
```cmd
# 依頼
- 上記文章から課題の整理と現実的な解決策を上司に提案したいので具体的な提案をするために、繰り返し深堀するための質問を一つずつ返してください。その際に選択肢を示す場合は、番号で答えられるようにしてください
- 私が「もうまとめて」とか「もう無理」と返したら、深堀して聞いたところはその分具体的な提案をしてください課題の整理と現実的な解決策を提案してください。深堀して質疑応答した部分に関しては、その内容を踏まえより具体的な提案としてください
```
...
- **制約**: 実装にむけたプロトタイプづくりが3日程度で終わる規模のシナリオの単位に分割してください
...
出された案に対して不満がある場合、もう一度考えてもらいましょう


```cmd
本当にこの案は最良ですか？もっと現実的に実装可能な案になるようにもう一度考え直してください。
もう少し（あなたの意見）を考慮してください。
...
# 生成AI活用シナリオ検討プロンプト　③口頭での文章記載方式

プロンプトをコピー後　（）内をあなたの情報に書き換えてください。
書き換える際に［Windows キー]　＋　［h］で音声入力をオンにして口頭記述を試してみてください。
①や②で実施したような、業種・職種・所属組織・業務目的・課題・活用可能なデータについて雑に話してみてください。
...
1.まず、上記前提条件の状況から、下記入力書式に沿って抽出し、ビジネスで活用可能な文章に変換してください
2.収集した情報に基づき課題を解決する生成AI活用方法を５つ提案して、どの解決方法に興味があるか番号で尋ねてください。
6.興味があると回答された解決方法に対して、下記のAI機能を用いて、下記出力形式に沿ってAI活用案を提示してください
